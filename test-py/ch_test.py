...
def polt_error_ch(n, times):
    ...
    time_qr = np.zeros((length, 1))
    time_qr_fix = np.zeros((length, 1))
    cumulate_time = np.zeros((length, 1))
    cumulate_time_qr = np.zeros((length, 1))
    cumulate_time_qr_fix = np.zeros((length, 1))
    ...
    for i in range(times):
        ...
        R_update_history = np.copy(R_updated)
        time1 = tm.time()
        ...
        cumulate_time[count] = cumulate_time[count - 1] + time[count]
        cumulate_time_qr[count] = cumulate_time_qr[count - 1] + time_qr[count]
        cumulate_time_qr_fix[count] = (
            cumulate_time_qr_fix[count - 1] + time_qr_fix[count]
        )
    ...
    plt.cla()
    plt.plot(time[:times])
    # plt.plot(time_qr[:times])
    plt.plot(time_qr_fix[:times])
    plt.xlabel("n")
    plt.ylabel("time")
